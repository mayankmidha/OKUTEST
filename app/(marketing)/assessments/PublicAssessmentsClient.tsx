...
export default function PublicAssessmentsClient({ isLoggedIn, assessments }: { isLoggedIn: boolean, assessments: any[] }) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          <AnimatePresence mode="popLayout">
            {filteredAssessments.map((a, index) => {
              ...
              const href = isLoggedIn ? `/assessments/${a.slug}` : `/auth/signup?callbackUrl=/assessments/${a.slug}`;
              return (
                <motion.div
                  key={a.id}
                  layout
        ...
        <SectionReveal className="mt-48">
            <div className="bg-oku-dark rounded-[4rem] p-16 md:p-24 lg:p-32 text-white relative overflow-hidden group shadow-2xl">
                <div className="relative z-10 max-w-4xl">
                    ...
                    <Link 
                        href={isLoggedIn ? "/assessments" : "/auth/signup"} 
                    ...
                    >
                        {isLoggedIn ? "Continue Exploring" : "Create Secure Record"} <ArrowRight size={16} />
                    </Link>
                </div>
