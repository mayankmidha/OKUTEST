  ...
  return (
    <div className="py-12 px-10">
      <DashboardHeader 
        ...
        actions={
          <Link href="/therapists" className="btn-primary py-4 px-10 flex items-center gap-2 shadow-xl hover:shadow-oku-purple/20 transition-all active:scale-95">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
        ...
        <DashboardCard subtitle="Today's Mood" icon={Sparkles} variant="dark">
          <p className="text-sm font-bold opacity-80 mb-1">
            {user.moodEntries[0] ? `Feeling ${user.moodEntries[0].mood >= 4 ? 'Better' : 'Quiet'}` : 'Start Tracking'}
          </p>
            ...
            <div className="space-y-4">
              {upcomingAppointments.length === 0 ? (
                <DashboardCard className="border-dashed py-20 text-center">
                  ...
                  <Link href="/therapists" className="text-oku-purple font-bold text-sm hover:underline mt-4 inline-block">Find a therapist to begin →</Link>
                </DashboardCard>
              ) : (
              ...
              {practitioners.map(p => (
                <Link key={p.id} href={`/book/${p.id}/trial`} className="flex items-center gap-4 group">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold text-current group-hover:text-oku-purple transition-colors truncate">{p.user.name}</p>
